...
- If no real project/repo markers are discoverable from the runtime context, do not hallucinate project state. Initialize a clearly-labeled scratch workspace instead, create `openspec/config.yaml` and `.atl/skill-registry.md` there, and report that the output is temporary until moved into a real repo.
- In that degraded mode, keep the config conservative: inferred stack only, `strict_tdd` based on evidence, and explicit notes that testing/project conventions were not discovered from a real repository.
